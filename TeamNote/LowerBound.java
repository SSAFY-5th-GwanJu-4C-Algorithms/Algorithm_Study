...
public class LowerBound {
    ...
    /**
     * lowerBound()를 수행하기 위한 준비 후, for문을 통해 0부터 10까지 lowerBound()의 결과를 출력한다.
     */
    private static void run() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            for (int k = 0; k <= 10; k++) {
                printHeader(k);
                int resultIndex = lowerBound(k, 0, length, arr);
     ...
     * <h3>Lower Bound</h3>
     * 
     * 정렬된 배열에서 KEY 이상의 값이 위치한 제일 작은 인덱스를 반환한다.
     * 
     * @param k - key : 배열에서 찾으려고 하는 값
     * @param l - left : 배열에서 찾고자 하는 범위의 왼쪽 값
     * @param r - right : 배열에서 찾고자 하는 범위의 오른쪽 값
     * @param a - array : 배열
     * @return KEY 이상의 값이 최초로 위치한 인덱스
     */
    private static int lowerBound(int k, int l, int r, int[] a) {
